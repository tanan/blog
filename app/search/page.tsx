...
export default async function Home({
  ...
  searchParams: { o: number; q: string };
}) {
  const o_fix = o === undefined ? 0 : o;
  const q_fix = q === undefined ? "" : q;
  const data = await fetch(
    `${process.env.NEXT_PUBLIC_API_ENDPOINT}/api/articles?offset=${o_fix}&q=${q_fix}`
  );
